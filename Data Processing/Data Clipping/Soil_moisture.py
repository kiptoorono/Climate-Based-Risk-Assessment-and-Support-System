...
def create_soil_moisture_time_series(base_dir, shapefile_path, start_year, end_year, excel_path):
    ...
    for year in range(start_year, end_year + 1):
        ...
        for month_dir in sorted(os.listdir(year_dir), key=lambda x: int(x)):  # Sort month directories as integers
            ...
            # Calculate mean of 'rfe_filled' along the time dimension (monthly average)
            mean_soilmoisture_monthly = combined_data["sm_c4grass"].mean(dim='time')

            # Set spatial dimensions and CRS
            mean_soilmoisture_monthly.rio.set_spatial_dims(x_dim='lon', y_dim='lat', inplace=True)
            mean_soilmoisture_monthly.rio.write_crs('epsg:4326', inplace=True)

            # Clip data to Kenya boundary using Level 3 shapefile
            try:
                clipped_data_monthly = mean_soilmoisture_monthly.rio.clip(wards.geometry.apply(mapping), wards.crs, drop=True)
                ...
                for ward in wards.itertuples():
                    ...
                    # Check if the clipped data is not empty
                    if clipped_ward_data.size == 0:
                        print(f"No data found for ward {ward_name} in {month_dir} {year}")
                        continue
                    ...
                    time_series_data[ward_name].append({
                        'Date': date_str,
                        'Soil_Moisture': ward_mean_rainfall
                    })
    ...
    pivot_df = time_series_df.pivot_table(index='Date', columns='Ward', values='Soil_Moisture')
...
# Parameters
base_dir = r'E:\Agriculture project\Data\Raw data\Soil Moisture'
shapefile_path = r'E:\Agriculture project\Data\gadm41_KEN_shp\gadm41_KEN_1.shp'
start_year = 1983
end_year = 2024
excel_path = r"E:\Agriculture project\Data\Semi-prepared data\Soilmoisture.xlsx"
